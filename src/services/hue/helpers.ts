...
export async function discoverAndCreateUser(
  ...
  const ipAddress = apiUrl ?? (await discoverBridge());
  if (!ipAddress) throw Error("Unable to discover Hue Bridge");
...
export async function connect(username: string, ipAddress: string) {
  // Create an unauthenticated instance of the Hue API so that we can create a new user
  const api = await hueApi.createLocal(ipAddress).connect(username);
  return new HueClient(api);
}
